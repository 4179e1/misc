...
void on_button_clicked(GtkButton *button, DataBaseBuilder *dbb)
{
	...
	char sql_statment_checked[MAX_LENGTH];
	...
	if((sql_statment_length = strlen(sql_statment)) > MAX_LENGTH - 2 )
	{
		gtk_text_buffer_insert(buffer, &iter, "Error: Sql statment too long\n", -1);
		return;
	}

	if( sql_statment[sql_statment_length - 1] != ';' )
	{
...
void on_debug_exec_sql_command_activate(GtkMenuItem *menuitem, DataBaseBuilder *dbb)
{
	...
	if(rc != SQLITE_OK) {
		...
		gtk_text_buffer_insert(buffer, &iter, dbb->database_name_temp, -1);
